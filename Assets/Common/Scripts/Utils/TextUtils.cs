...
public class TextUtils {
    ...
    /** Use me if you wanna know if the word BEFORE this one should be "a" or "an". */
    public static bool StartsWithVowel(string str) {
        if (string.IsNullOrWhiteSpace(str)) return false; // Safety check.
        str = str.ToLower();
        char c = str[0];
    ...
    /** E.g. 83.45 returns "1:23.45", 50.2 returns "0:50", etc. */
    public static string GetSecondsToTimeString (float _totalSeconds) {
        int secondsInt = Mathf.FloorToInt(_totalSeconds);
        return $"{secondsInt / 60}:{secondsInt % 60:D2}";
...
//      return string.Format ("{0}:{1:00}:{00}", (int)timeFloat / 60, (int)timeFloat % 60, timeFloat%1);
    ...
    // Converts "1.2345678" to "1.23"
    static public string DecimalPlaces2(float val) {
        return (Mathf.Round(val*100f)/100f).ToString();
    }
...
//      return Regex.Replace(_string, @"\s+", "");public static string RemoveWhitespace(this string str) {
    ...
    /** Pass in like this: "-100,320,42". */
    static public Vector3 GetVector3FromString(string str) {
        // NOTE: This is UNTESTED! I think it'll work but idk.
        if (str==null) { return Vector3.zero; }
        str = str.Substring(1, str.Length-2); // cut the parenthesis.
        int indexOfComma0 = str.IndexOf(',');
        int indexOfComma1 = str.IndexOf(',', indexOfComma0+1);
        string xString = str.Substring(0, indexOfComma0);
        string yString = str.Substring(indexOfComma0+2, indexOfComma1-indexOfComma0-2); // starting after ", ".
        string zString = str.Substring(indexOfComma1+2); // starting after ", ".
        try { // test
            float x = ParseFloat(xString);
            float y = ParseFloat(yString);
            float z = ParseFloat(zString);
            return new Vector3(x, y, z);
        }
        catch {
            Debug.Log("Error parsing Vector3 string. x: " + xString + ", y: " + yString + ", z: " + zString);
            return Vector3.zero;
        }
    }
